            ...
            function($scope, $routeParams,$location,crudService,socket,$filter,$route,$log){
               ...
               if($location.path() == '/otherPheads/show/'+$routeParams.id) {
               	...
               	crudService.byId(id,'pagos2').then(function (data) {                	 	 
                       ...
                       $scope.otherPhead.saldo=Number($scope.otherPhead.Saldo);
                    });
                   	 });
                   	  $scope.tot=0
                ...
                $scope.createPagos = function(){
                    ...
                    if($scope.pagos.cajamensual==true){
                        $scope.pagos.fecha1=$scope.pagos.fecha.getFullYear()+'-'+($scope.pagos.fecha.getMonth()+1)+'-'+$scope.pagos.fecha.getDate()+' '+$scope.otherPhead.fecha.getHours()+':'+$scope.otherPhead.fecha.getMinutes()+':'+$scope.otherPhead.fecha.getSeconds();
                       
                    }else{
                    	$scope.pagos.fecha1=$scope.pagos.fecha.getFullYear()+'-'+($scope.pagos.fecha.getMonth()+1)+'-'+$scope.pagos.fecha.getDate();
                    	$scope.pagos.hora.setUTCHours($scope.otherPhead.fecha.getHours());
                    }
